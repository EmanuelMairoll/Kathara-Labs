...
# Use a shared working directory
WORKDIR="/shared/scion-certs"

# Clean the work directory if it already exists, then recreate it
if [ -d "$WORKDIR" ]; then
    echo "Cleaning existing work directory: $WORKDIR"
    #rm -rf "$WORKDIR"
    echo "$WORKDIR"
fi
...
# Create AS directories for all five hosts
mkdir -p AS{1..5}

# --- Generate certificates for AS1 ---
pushd AS1
...
[validity]
not_before = '$(date +%s)'
validity = "365d"' > trc-B1-S1-pld.tmpl
...
echo "SCION certificates generated in ${WORKDIR}"
